...
PostgreSQL has no way to find rows with `last_name = 'Smith'` without reading every single row in the `customers` table. It performs a **Sequential Scan** — checking all 50,000 rows one by one and discarding the ones that don't match.
...
PostgreSQL builds a sorted B-tree of all last name values. Now when you search for `'Smith'`, it can look up the value in the tree and go directly to the matching rows — without reading the other 49,990+ rows.
...
- **Equality predicate**: `= 'Smith'` is the simplest and most efficient type of index lookup.
